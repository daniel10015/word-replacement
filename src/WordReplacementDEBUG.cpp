...
void Recorder::importCSV()
{
    ...
    while(getline(fi, line))
    {
        ...
        for(int i=0; i<lengthOfLine; i++)
        {
            if(line.substr(i,1) == ";")
            {
                cout << toUpper(line.substr(0,i)) << " -> " << toUpper(line.substr(i+1,lengthOfLine-i)) << endl;
                this->_wordPairs[toUpper(line.substr(0,i))] = toUpper(line.substr(i+1,lengthOfLine-i));
                i = lengthOfLine;
            }
        }
    }
